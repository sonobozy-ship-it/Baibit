...
# ВАЖНО: символы НЕ должны пересекаться с S1-S14 (вызывает блокировку корреляцией).
# S1=BTC S2=ETH S4=BNB S5=DOGE S6=XRP S8=DOT S9=NEAR S11=AVAX
SCALP_SYMBOLS = [
    # Пользовательские фавориты (гарантированно создаются)
    "JASMYUSDT",       # SC_JASM — высокий объём, volatile meme
    "1000SHIBUSDT",    # SC_SHIB — micro-lot, высокая волатильность
    "BILLIEUSDT",      # SC_BILL — активный meme token
    # Дополнительные ликвидные символы
    "TRXUSDT",         # ~$0.12/лот, ликвидный
    "XLMUSDT",         # ~$0.11/лот, ликвидный
    "VETUSDT",         # ~$0.03/лот, много объёма
    "LTCUSDT",         # ~$80/лот, высокая ликвидность
    "ATOMUSDT",        # ~$4/лот, ликвидный
    "ARBUSDT",         # ~$0.50/лот, волатильный L2
    "SUIUSDT",         # ~$2/лот, волатильный
    "FTMUSDT",         # ~$0.50/лот, высокая волатильность
    "INJUSDT",         # ~$15/лот, ликвидный на Bybit
]
...
class ScalperProStrategy(BaseStrategy):
    ...
    _BB_PERIOD  = 20
    _BB_K       = 2.0
    _VOL_MULT   = 1.3   # объём должен быть > 1.3× среднего (фильтр мёртвого рынка)
